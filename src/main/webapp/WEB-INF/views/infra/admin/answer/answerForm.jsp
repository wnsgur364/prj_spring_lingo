	...
	<c:when test="${empty param.seq }"> 
     ...
     <c:otherwise>      
           ...
           <div class="form-group">
            ...
            <div class="form-group">
            <label for="input-3">answerNy</label>
            <input type="text" class="form-control" name="answerNy" id="answerNy" value="<c:out value="${item.answerNy }"/>" >
           </div>
            <div class="form-group">
           ...
           <div class="form-group">
            <label for="input-3">answer</label>
            <input type="text" class="form-control" name="quiz_seq" id="quiz_seq" value="<c:out value="${item.quiz_seq }"/>" >
           </div>
...
$("#btnSave").on("click", function(){
	...
	$("form[name=form]").attr("action", "/answerInst").submit();
	
});

$("#btnUpdate").on("click", function(){
 	$("form[name=form]").attr("action", "/answerUpdt").submit();
});


$("#btnDelete").on("click", function(){
 	$("form[name=form]").attr("action", "/answerDele").submit();
});
